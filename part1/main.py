...
# 6. Changing Labels
#######################################################################
#test error on mold 3
# (train_y_3,test_y_3) = update_y(train_y, test_y)
# theta, cost_function_history = softmax_regression(train_x, train_y, temp_parameter = 1, alpha=0.3, lambda_factor=1.0e-4,
#                                                   k=10, num_iterations=150)
# test_error_3 = compute_test_error_mod3(test_x, test_y_3, theta, temp_parameter=1)
#
...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    train_x, train_y, test_x, test_y = get_MNIST_data()
    train_y_3, test_y_3 = update_y(train_y, test_y)
    train_x_3, test_x_3 = update_y(train_x, test_x)
    theta, cost_function_history = softmax_regression(train_x_3, train_y, temp_parameter, alpha=0.3, lambda_factor=1.0e-4,
                                                      k=10, num_iterations=150)
    plot_cost_function_over_time(cost_function_history)
    test_error = compute_test_error(test_x_3, test_y, theta, temp_parameter)
    # Save the model parameters theta obtained from calling softmax_regression to disk.
    write_pickle_data(theta, "./theta.pkl.gz")
    # TODO: add your code here for the "Using the Current Model" question in tab 4.
    #      and print the test_error_mod3
...
# train_cube = cubic_features(train_cube)
# test_cube = cubic_features(test_cube)
...
def run_softmax_on_MNIST_cubic(temp_parameter=1):
    ...
    return test_error
print('softmax test_error cubic', run_softmax_on_MNIST_cubic(temp_parameter=1))
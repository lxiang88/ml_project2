...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    # print(pcs_n,'pcs_n')
    # for i in range(len(pcs_n[0])):
    #     print(pcs_n[:,i],'pcs_1')
    #     n_i = np.dot(X_centered,pcs_n[:,i])
    #     print(n_i,'n_i')
    #     project = np.append(project,n_i)
    # print (project,'project')
    # return project.reshape(len(X),n_components)
    return np.dot(X_centered,pcs_n)

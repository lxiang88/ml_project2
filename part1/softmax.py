...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    # c = theta[len(theta)-1].dot(X.transpose())/temp_parameter
    # e = (np.dot(theta,X.transpose())/temp_parameter-c)
    # h = np.exp(e)
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    k = len(theta)
    n = len(Y)
    s = sparse.coo_matrix(([1] * n, (Y, range(n))), shape=(k, n)).toarray()

    p = np.log(np.clip(compute_probabilities(X, theta, temp_parameter), 0, 1))


    error_term = np.sum(p[s == 1]) / (-1 * n)
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    # s = sparse.coo_matrix(([1] * n, (Y, range(n))), shape=(k, n)).toarray()
    #
    # trans = s - compute_probabilities(X, theta, temp_parameter)
    #
    #
    # theta_f=[]
    # for i in range(len(theta)):
    #     theta_i = trans[i]
    #     theta_i_af = np.sum(np.multiply(X, theta_i[:, np.newaxis]),axis=0)/((-1) *n*temp_parameter) + lambda_factor * theta[i]
    #
    #     theta_f.append(theta_i_af)
    #
    # theta_f = np.array(theta_f)
    # return theta - theta_f * alpha
